...
public class NumberAssignatorFunction
{
    ...
    public void Run(
        ...
        ILogger log,
        [ServiceBus("registration-completed", Connection = "ServiceBusConnectionString")] out ServiceBusMessage outputMessage)
    {
        outputMessage = null;
        ...
        try
        {
            ...
            transaction.Commit();
            SendNotification(registerCompetitor, number).Wait();
            ...
            outputMessage = new ServiceBusMessage(new BinaryData(System.Text.Json.JsonSerializer.Serialize(integrationEvent)));
        }
        catch (Exception ex)
        {
